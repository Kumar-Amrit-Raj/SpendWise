...
export default function Recurring({ refreshKey, onChanged }) {
  ...
  return <section><div className="section-head"><div><h2>Recurring transactions</h2><p className="muted">Automatically create due entries without duplicates.</p></div></div>
    ...
    <div className="list">{items.map(x=><article className={`list-row recurring-row ${x.type}`} key={x._id}><div><div className="row"><strong>{x.title}</strong><span className={`pill ${x.type}`}>{x.type}</span></div><small>{x.category} · {x.frequency} · next {new Date(x.nextRun).toLocaleDateString('en-IN')}</small></div><div className="row"><strong className={`semantic-amount ${x.type}`}>{x.type === 'expense' ? '-' : '+'}{INR.format(x.amount)}</strong><button onClick={()=>toggle(x)}>{x.active?'Pause':'Resume'}</button><button onClick={()=>remove(x._id)}>Delete</button></div></article>)}</div>
  </section>;
}
